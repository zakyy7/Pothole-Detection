...
if mulai_btn:
    if not (uploaded_model and uploaded_video and uploaded_gps):
        ...
        # VARIABEL BARU UNTUK MENGHITUNG JARAK ANTAR LUBANG
        last_pothole_coords = None
                        ...
                        if track_id not in counted_ids:
                            if row is not None:
                                ...
                                if is_really_new:
                                    ...
                                    # ========================================================
                                    # LOGIKA BARU: HITUNG JARAK DENGAN LUBANG SEBELUMNYA
                                    # ========================================================
                                    jarak_antar_lubang = 0.0
                                    if last_pothole_coords is not None:
                                        jarak_antar_lubang = geodesic(last_pothole_coords, current_coords).meters
                                    last_pothole_coords = current_coords
                                    # ========================================================
                                    ...
                                    detected_data.append({
                                        'ID Lubang': track_id, 
                                        'Waktu Asli': current_time.strftime('%H:%M:%S'),
                                        'Waktu Offset': target_time.strftime('%H:%M:%S'),
                                        'Latitude': row['Fixed_Lat'],
                                        'Longitude': row['Fixed_Lon'],
                                        'Speed': row['Speed'],
                                        'Conf': conf,
                                        'Jarak Lubang Sebelumnya (m)': round(jarak_antar_lubang, 2), # KOLOM BARU
...
if st.session_state.proses_selesai and st.session_state.df_hasil is not None:
    ...
    for _, r in df_out.iterrows():
        ...
        <div style="font-family: Arial; width: 320px;">
            <b style="font-size: 14px;">Lubang ID: {r['ID Lubang']}</b><br>
            <span style="font-size: 12px; color: gray;">Waktu: {r['Waktu Asli']} | Conf: {r['Conf']:.2f}</span><br>
            <span style="font-size: 12px; color: blue;">Jarak dr sblmnya: {r['Jarak Lubang Sebelumnya (m)']} m</span>
